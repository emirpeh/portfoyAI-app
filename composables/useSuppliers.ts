...
export function useSuppliers() {
  ...
  async function fetchSuppliers(page = 1, limit = 10) {
    try {
      loading.value = true
      error.value = null
      const response = await $apiFetch<{ data: Supplier[], pagination: Pagination }>(`/suppliers?page=${page}&limit=${limit}`)
    ...
    try {
      ...
      await $apiFetch('/suppliers', {
        ...
        body: {
          ...
          foreignTrades: Array.isArray(supplier.foreignTrades) ? supplier.foreignTrades.join(',') : supplier.foreignTrades,
        },
      })
    ...
    try {
      ...
      await $apiFetch(`/suppliers/${id}`, {
        ...
        body: {
          ...
          foreignTrades: supplier.foreignTrades ? (Array.isArray(supplier.foreignTrades) ? supplier.foreignTrades.join(',') : supplier.foreignTrades) : undefined,
        },
      })
      await fetchSuppliers(pagination.value.page, pagination.value.limit)
    }
  ...
  async function bulkAddSuppliers(suppliersData: any[]) {
    const { $apiFetch } = useNuxtApp()
    try {
      loading.value = true
      error.value = null
